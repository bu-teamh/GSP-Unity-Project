...
namespace GSP.States
{
	...
	public class FuelGameplayState : NormalGameplayState
	{
		//Constructor doesn't need touching.
		public FuelGameplayState(GameStateManager _object) : base(_object) { }

		//Constructor doesn't need touching.
		public FuelGameplayState(BaseState _state) : base(_state) { }
		...
		public override void Update()
		{
			...
			if (
				!(m_localEnemies.Count > 0) &&
				m_thisObject.m_component.GetGlobalValue(GlobalValue.InventoryHealth) > 0 &&
				m_companion.GetState() == typeof(CompanionFollowState)
				)
			{
				m_thisObject.m_component.AddToGlobalValue(GlobalValue.InventoryFuel, -1);
				SendEvent(EventPriority.Routine, EventArchetype.Gameplay, EventSubtype.Translate, EventFlag.Active);
			}
