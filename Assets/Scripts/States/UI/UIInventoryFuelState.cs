...
namespace GSP.States
{
	...
	public class UIInventoryFuelState : UIBaseState
	{
		//Constructor doesn't need touching.
		public UIInventoryFuelState(UIManager _object) : base(_object) { }

		//Constructor doesn't need touching.
		public UIInventoryFuelState(BaseState _state) : base(_state) { }
		...
		protected override void InitializeMap()
		{
			...
			SetTransition(typeof(UIInventoryReviveState), EventArchetype.Internal, EventSubtype.Menu, EventFlag.Revive);
		...
		public override void React(GameEvent _event)
		{
			...
			{
				InternalEvent(EventSubtype.Menu, EventFlag.Revive);
			}
			else if (
				CompareEvent(_event, EventArchetype.MenuInput, EventSubtype.Interact, EventFlag.KeyDown))
			{
				...
				SendEvent(EventPriority.Routine, EventArchetype.Gameplay, EventSubtype.UseItem, EventFlag.Fuel);
			}
		}
