...
namespace GSP.States
{
	...
	public class UIInventoryHealthState : UIBaseState
	{
		//Constructor doesn't need touching.
		public UIInventoryHealthState(UIManager _object) : base(_object) { }

		//Constructor doesn't need touching.
		public UIInventoryHealthState(BaseState _state) : base(_state) { }
		...
		protected override void InitializeMap()
		{
			...
			SetTransition(typeof(UIInventoryFuelState), EventArchetype.Internal, EventSubtype.Menu, EventFlag.Fuel);
		...
		public override void React(GameEvent _event)
		{
			...
			{
				InternalEvent(EventSubtype.Menu, EventFlag.Fuel);
			}
			else if (
				CompareEvent(_event, EventArchetype.MenuInput, EventSubtype.Interact, EventFlag.KeyDown))
			{
				...
				SendEvent(EventPriority.Routine, EventArchetype.Gameplay, EventSubtype.UseItem, EventFlag.Health);
			}
		}
